...
//template <typename unsigned int>
__device__ inline
void bitonicSort(unsigned int *pos1, unsigned int *pos2, unsigned int *shared, unsigned int * sharedpos, unsigned int* fromvalues, unsigned int* tovalues, unsigned int from, unsigned int size)
{
				...
				if (ixj > tid)
				{
					if ((tid & k) == 0)
					{
						if (shared[tid] > shared[ixj])
						{
							...
							swap(sharedpos[tid], sharedpos[ixj]);
						}
					}
					else
					{
...
__global__ void part2(unsigned int * pos, unsigned int * pos2, unsigned int* data, unsigned int* data2, struct Params<unsigned int>* params, struct Hist* hist, Length<unsigned int>* lengths)
{
	const int tx = threadIdx.x;
	...
	if(tx+start<end)
	{
		...
		if(d<=pivot)
			{ data2[x]=d; pos2[x]=pos[tx+start];x--;}
		else
			{ data2[y]=d; pos2[y]=pos[tx+start];y++;}
		}
	}
	...
	for(unsigned int i=start+tx+THREADS;i<end;i+=THREADS)
	{
		...
		// and writing them to auxillary array
		if(d<=pivot)
		{		data2[x]=d;    pos2[x]=pos[i];x--;		}
		else
		
			{ data2[y]=d; pos2[y]=pos[i];y++; }
	}
...
**-------------------------------------------------------------
**-------------------------------------------------------------
* promedio, acumula el promedio
*/
__global__ void promedio(unsigned int* data, float * cum, unsigned int size)
{
        const unsigned int t= size/gridDim.x+1;
	const unsigned int tx = blockIdx.x*t+threadIdx.x;
        unsigned int x;
       
        for(x=tx ; x<tx+t; x+=THREADS) if(x<size) cum[x]+=(float)data[x];
}
...
__global__ void divide(float* data, float n, unsigned int size)
{
        const unsigned int t= size/gridDim.x+1;
	const unsigned int tx = blockIdx.x*t+threadIdx.x;
        unsigned int x;
       
        for(x=tx ; x<tx+t; x+=THREADS) if(x<size) data[x]/=n;
}

/**-------------------------------------------------------------
**-------------------------------------------------------------
* promedio, acumula el promedio
__global__ void init_pos(unsigned int* data, unsigned int size)
{
        const unsigned int t= size/gridDim.x+1;
	const unsigned int tx = blockIdx.x*t+threadIdx.x;
        unsigned int x;
       
        for(x=tx ; x<tx+t; x+=THREADS) if(x<size) data[x]=x;
}
*/
...
*/
//template <typename unsigned int>
__global__ void lqsort(unsigned int *apos1, unsigned int * apos2, unsigned int* adata, unsigned int* adata2, struct LQSortParams* bs, unsigned int phase)
{
	...
	__shared__ bool second[32];

//	unsigned int* lblock = (unsigned int*)sarray;
//	unsigned int* rblock = (unsigned int*)(&lblock[(blockDim.x+1)]);
	__shared__ unsigned int rblock[1024];
        __shared__ unsigned int lblock[1024];
	...
	__shared__ unsigned int* pos1;
	__shared__ unsigned int* pos2;
	...
	while(bi>=0)
	{
		...
		if((to-from)<(sbsize-16))
		{
			...
			if((to-from>=1)&&(lphase!=2))
				bitonicSort(pos1,apos1,rblock,lblock,data,adata,from,to-from);
			__syncthreads();
        ...
        if(second[bi])
		{
			if(rblock[THREADS]==0) 
			{
				...
				if(tx+from-coal<to && !(tx<coal))
				{
					adata[tx+from-coal] =  data[tx+from-coal];
					apos1[tx+from-coal] =  pos1[tx+from-coal];
				}
			...
			else  // lblock!=0
			{
				...
				if(tx+from-coal<to)
				{
					...
					if(!(tx<coal))
					{
						if(d<=pivot)
						{data2[x] = d; 
						pos2[x]=pos1[tx+from-coal]; 
						x--;}
						else
						{data2[y] = d; 
						pos2[y]=pos1[tx+from-coal]; 
						y++;}
					}	
				}
		...
		else  // !second
		{
			...
			{
				if(tx==0) {
					second[bi]=1; // all items are same value
					//                      pos1[count*4+3]=1;
				}			
				...
				if(tx+from-coal<to && !(tx<coal))
				{
					data2[tx+from-coal] =  data[tx+from-coal];
					pos2[tx+from-coal] =  pos1[tx+from-coal];
				}
